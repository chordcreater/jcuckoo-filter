...
public class CuckooFilter<T> {
    ...
    */
     ...
     */
    public CuckooFilter(Long estimatedMaxNumKeys){
        // 计算出实际的桶的个数
        long numBuckets = getBucketsNeeded(estimatedMaxNumKeys, LOAD_FACTOR, BUCKET_SIZE);
    ...
    private ItemPos generateIndexTagHash(T item){
        ...
        itemPos.setTag(tag);
        Long altIndex = table.altHashIndex(curIndex, tag);
        itemPos.setAltIndex(altIndex);
        return itemPos;
    }
